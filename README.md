...
```yml
    ...
    - name: Terraform Init
      id: init
      run: terraform init -backend-config="bucket=$TF_STATE_BUCKET_NAME" -backend-config="region=ca-central-1"
...
- name: Deploy Docker Image to EC2
  ...
  with:
    ...
    script: |-
      ...
      sudo $(aws ecr get-login --no-include-email --region ca-central-1);
